...
public class RlvConversationAdapter extends RecyclerView.Adapter<RlvConversationAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder viewHolder, final int i) {
        Glide.with(activity).load(list.get(i).getTopicPicture()).transform(new CenterCrop(),new GlideRoundTransform(activity,0)).placeholder(R.mipmap.bg).into(viewHolder.mIvShow);
